    ...
    void writetooutput(){
        ...
        for(int i=0;i<m;i++){
            ...
            for(int j=0;j<n;j++){
                ...
                int len = itemmap[transactions[i][j]].size();
                for(int k=0;k<len;k++)temp += to_string(itemmap[transactions[i][j]][k])+" ";
            }
            buffer_insert(temp);
        }
        write();
...
int main(){
    string fileLocation = "/home/groot/Desktop/Coding/A1_datasets/output.dat";
    string outputlocation = "/home/groot/Desktop/Coding/A1_datasets/kc.dat";
