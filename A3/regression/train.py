...
def main():
    ...
    #We create the optimizer to optimize th evalues 
    optimizer = torch.optim.Adam(par, lr=learning_rate, weight_decay = decay)
    ...
    # The below is the llist of validation
    Validation_Metric_error= []

    #Now this the learing phase

    n_epoch = 100#epochs
    for epoch in range(n_epoch):
        #calculating trhe initail loss
        loss = train(model,train_loader,optimizer,criterion)
        #Train error
        train_error = test(model,train_loader,criterion)
        #testing error 
        test_error = test(model,test_loader,criterion)
        if temp==-1 or temp>test_error:
            temp = test_error
            ans = copy.deepcopy(model)
        
        # Training_Metric_error.append(train_error)
        #getting the validation list
        # Validation_Metric_error.append(test_error)
        #Now we are printing the values in the terminal
        print(f'Epoch: {epoch:03d}, Train Acc: {train_error:.4f}, Test Acc: {test_error:.4f}')
    ...
    # value_of_epochs = [i for i in range(100)]
    # # Set the title of the plot
    # plotting_my_graph_object.title('RMSE vs Epochs')

    # # Set the label for the x-axis
    # plotting_my_graph_object.xlabel('Epochs')

    # # Set the label for the y-axis
    # plotting_my_graph_object.ylabel('RMSE')

    # # Plot the Training data with a label for the legend
    # plotting_my_graph_object.plot(value_of_epochs, Training_Metric_error, label='Training',color='red')

    # # Plot the Validation data with a label for the legend
    ...
    # plotting_my_graph_object.savefig('Regression_learning_curve.png')
